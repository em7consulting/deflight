...
public class CrazyAirFlightClient implements FlightClient {
    ...
    public CrazyAirFlightClient(URI uri,
    ...
    public List<Flight> getFlight(FlightSearchCriteria flightSearchCriteria) throws ExternalApiException {
        try {
            ...
            return Arrays.stream(objectMapper.readValue(httpResponse.body(), CrazyAirResponse[].class))
                    .map(crazyAirResponse -> crazyAirResponse.toFlight(airportZoneIdResolver::resolve))
                    .toList();
        } catch (Exception e) {
            throw new ExternalApiException("There was a problem with crazy air API");
        }
    }
    ...
    private Callable<HttpResponse<String>> sendRequestSafely(FlightSearchCriteria flightSearchCriteria) throws JsonProcessingException {
        final var request = HttpRequest.newBuilder(uri.resolve("/crazy-air/flight-info"))
                .POST(HttpRequest.BodyPublishers.ofString(objectMapper.writeValueAsString(CrazyAirRequest.from(flightSearchCriteria))))
                .build();
